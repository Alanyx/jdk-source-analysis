...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * 有了SUID之后，那么如果序列化的类已经保存了在本地中，中途你更改了类后，SUID变了，那么反序列化的时候就不会变成原始的类了，
     * 还会抛异常，主要就是用于版本控制
     */
    //序列号
     ...
     */
    // 最大容量：约为10亿级别（必须是2的幂且小于2的30次方，传入容量过大将被这个值替换）
     ...
     * shrinkage.
     */
    // 树的临界值: 当put一个元素到某个桶位，其链表长度达到8时将链表转换为红黑树
    // todo 2<size<=8 使用红黑树来实现 什么是红黑树？
     ...
     */
    // 非树的临界值:一个桶位上的链表长度小于这个值时将红黑树转链表
    ...
    // 然后为了避免冲突(容量在resizing 和 treeification thresholds之间) 设置成64
     ...
     */
    // 这里在原生hashCode的基础上做了一次与高16位相异或的处理，
     ...
     * @throws IllegalArgumentException if the initial capacity is negative.
     */
    //Tips:用这种构造函数创建HashMap的对象，如果知道map要存放的元素个数，可以直接指定容量的大小， 减除不停的扩容，提高效率
     ...
     */
    // todo 注意如果这个map之前包含这个key的映射，旧的value将被新值替换。
